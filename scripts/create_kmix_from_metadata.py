...
parser.add_argument('--metadata_dir', type=str, required=False, default='metadata/Kspon',
                    help='Path to the ksponspeech metadata directory')
parser.add_argument('--kspon_outdir', type=str, default='storage_dir',
...
def main(args):
    ...
    metadata_dir = metadata_dir +'{}mix'.format(args.n_src)

    kspon_outdir = args.kspon_outdir
    n_src = args.n_src

    kspon_outdir = os.path.join(kspon_outdir, f'kspon{n_src}Mix')
    os.makedirs(kspon_outdir, exist_ok=True)
    ...
    create_kmix(kspon_dir, wham_dir, kspon_outdir, metadata_dir,
                    freqs, n_src, modes, types)


def create_kmix(kspon_dir, wham_dir, out_dir, metadata_dir,
                    freqs, n_src, modes, types):
    """ Generate sources mixtures and saves them in out_dir"""
    # Get metadata files
    md_filename_list = [file for file in os.listdir(metadata_dir)
                        if 'info' not in file]
    # Create all parts of ksponspeech
    for md_filename in md_filename_list:
        csv_path = os.path.join(metadata_dir, md_filename)
        process_metadata_file(csv_path, freqs, n_src, kspon_dir,
                              wham_dir, out_dir, modes, types)


def process_metadata_file(csv_path, freqs, n_src, kspon_dir, wham_dir,
                          out_dir, modes, types):
    ...
    for freq in freqs:
        ...
        for mode in modes:
            ...
            dir_name = os.path.basename(csv_path).replace(
                f'Kspon{n_src}mix_', '').replace('-clean', '').replace(
                '.csv', '')
            ...
            process_utterances(md_file, kspon_dir, wham_dir, freq, mode,
                               subdirs, dir_path, subset_metadata_path, n_src)

def process_utterances(md_file, kspon_dir, wham_dir, freq, mode, subdirs,
    ...
    for results in tqdm.contrib.concurrent.process_map(
        functools.partial(
            process_utterance, 
            n_src, kspon_dir, wham_dir, freq, mode, subdirs, dir_path),
...
def process_utterance(n_src, kspon_dir, wham_dir, freq, mode, subdirs, dir_path, row):
    res = []
    # Get sources and mixture infos
    mix_id, gain_list, sources = read_sources(row, n_src, kspon_dir,
                                              wham_dir)
    # Transform sources
...
def read_sources(row, n_src, kspon_dir, wham_dir):
    ...
    for sources_path in sources_path_list:
        sources_path = os.path.join(kspon_dir,
                                    sources_path)
